...
export default function FareCalculator({ onProceedToPayment }) {
  ...
  return (
    <div style={styles.page}>
      <div style={styles.card}>
        ...
        {/* Distance Input */}
        <div style={styles.section}>
          <label style={styles.label} htmlFor="distance">Distance (km)</label>
          ...
          <div style={styles.resultBox}>
            ...
            {result.peakApplied && (
              <div style={styles.peakBanner}>
                ⚡ {result.peakLabel} — {result.peakMultiplier}× surcharge applied
              </div>
            )}
            ...
            <div style={styles.resultRows}>
              ...
              <Row label="Subtotal"        value={`RM ${result.subtotal.toFixed(2)}`} />
              {result.peakApplied && (
                <Row label={`Peak Surcharge (×${result.peakMultiplier})`} value={`RM ${(result.totalFare - result.subtotal).toFixed(2)}`} peak />
              )}
              <div style={styles.divider} />
              <Row label="Total Fare" value={`RM ${result.totalFare.toFixed(2)}`} highlight />
            </div>

            {/* Proceed to Payment */}
            <button
              style={styles.btnPayment}
              onClick={() => onProceedToPayment(result)}
            >
              Proceed to Payment →
            </button>
          </div>
...
function Row({ label, value, highlight, peak }) {
  return (
    <div style={styles.row}>
      <span style={highlight ? styles.rowLabelBold : peak ? styles.rowLabelPeak : styles.rowLabel}>{label}</span>
      <span style={highlight ? styles.rowValueBold : peak ? styles.rowValuePeak : styles.rowValue}>{value}</span>
    </div>
  );
}
...
const styles = {
  ...
  vehicleBtnActive: { border: "2px solid #2563eb", background: "#eff6ff" },
  ...
  inputUnit: { position: "absolute", right: "14px", fontSize: "13px", color: "#94a3b8", fontWeight: 500 },
  errorBox: {
  ...
  peakBanner: {
    ...
    fontWeight: 600, marginBottom: "14px", textAlign: "center",
  },
  resultBox: {
    background: "#f0f9ff", border: "1px solid #bae6fd", borderRadius: "14px", padding: "20px",
  },
  ...
  rowLabelPeak: { fontSize: "13px", color: "#854d0e" },
  rowValuePeak: { fontSize: "13px", color: "#854d0e", fontWeight: 600 },
